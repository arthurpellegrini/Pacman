...
char *NOM_POLICE;
char *NOM_POLICE1 = "./lib/verdana.ttf" ;
char *NOM_POLICE2 = "../lib/verdana.ttf" ;
...
void ouvrir_fenetre(int largeur, int hauteur){
    FILE *f=fopen(NOM_POLICE1,"r");
    if(!f)
        {
        f=fopen(NOM_POLICE2,"r");
        if(!f)
            {
            fprintf(stderr,"Erreur: les chemins '%s' et '%s' ne permettent pas de trouver le dossier 'lib' et la police 'verdana.ttf'\n",NOM_POLICE1, NOM_POLICE2);
            exit(1);
            }
        else
            NOM_POLICE = NOM_POLICE2;
        }
    else
        NOM_POLICE = NOM_POLICE1;
    fclose(f);
...
// affiche l'image sous forme .bmp (bitmap) en rendant une couleur donnée transparente
// nom est une chaine de caracteres qui est le nom (complet) du fichier image
// coin est le coin haut, gauche voulu pour l'image à afficher dans l'ecran
// couleur est la Couleur à rendre transparente
// Fonction proposée par Aurélien Marc FA2 2020
void afficher_image_transparente(char *nom, Point coin, Couleur couleur){
    int r,v,b;
    b = couleur%256;
    couleur >>= 8;
    v = couleur%256;
    ...
    SDL_Surface *imgcharge = SDL_LoadBMP(nom) ;
    SDL_SetColorKey( imgcharge, SDL_SRCCOLORKEY, SDL_MapRGB(imgcharge->format, r, v, b));
    SDL_Rect position_img ;
    position_img.x = coin.x;
    position_img.y = coin.y;
    SDL_BlitSurface(imgcharge,NULL,ecran,&position_img);
}
...
/* ICI, modif suggérée par Bilel Medimegh FI1 2020 pour prise en compte des accents    */
